...
template <int H, int W> __global__ void softargmax_kernel_half_depth(const __half2 *srcData, 
   __half *dstDataX,
   __half *dstDataY,
   __half *dstDataZ,
   __half *dstDataProb,
   ...
   __half epsilon)
{
    ...
    __shared__ __half s_sumDepths[H];
    ...
    const int elemOffset = (batch * 2 * CHW + channel * HW + row * W)/2;
...
#pragma unroll
    ...
    __half2 sumDepth = __float2half2_rn(0.0f);
    const int depthOffset = elemOffset + CHW / 2;
...
#pragma unroll
    for (int colIdx = 0; colIdx < W/2; ++colIdx)
    {
        ...
        sumDepth = __hfma2(prob, srcData[depthOffset + colIdx], sumDepth);
    }
    ...
    s_sumDepths[row] = __hadd(__low2half(sumDepth), __high2half(sumDepth));
    __syncthreads();
    ...
    if (row == 0)
    {
        ...
        __half sumChannelZ = __float2half(0.0f);
...
#pragma unroll
        for (int rowIdx = 0; rowIdx < H; ++rowIdx)
        {
            ...
            sumChannelZ += s_sumDepths[rowIdx];
        }
        // Store the outputs in global memory:
        dstDataX[channel] = sumChannelX;
        dstDataY[channel] = sumChannelY;
        dstDataZ[channel] = sumChannelZ;
    }

}

template <int H, int W> __global__ void softargmax_kernel_float_depth(const float *srcData, 
   float *dstDataX,
   float *dstDataY,
   float *dstDataZ,
   float *dstDataProb,
   ...
   float epsilon)
{
    ...
    __shared__ float s_sumDepths[H];
...
#pragma unroll
    ...
    const int depthOffset = elemOffset + CHW;
    for (int colIdx = 0; colIdx < W; ++colIdx)
    {
        // Grab an element pair from the local buffer, computed from the previous step:
        float elem = dataLocal[colIdx];
        ...
        // Calculate the pixel-wise multiplication between prob and depth
        sumDepth = fmaf(prob, srcData[depthOffset + colIdx], sumDepth);

    }
    s_sumRows[row] = sumRowsX;
    s_sumCols[row] = sumRowsY;
    s_sumDepths[row] = sumDepth;
    __syncthreads();
...
#pragma unroll
        for (int rowIdx = 0; rowIdx < H; ++rowIdx)
        {
            ...
            sumZ += s_sumDepths[rowIdx];
        }
        // Store the outputs in global memory:
        dstDataX[channel] = sumX;
        dstDataY[channel] = sumY;
        dstDataZ[channel] = sumZ;
    }

}

// C should be 21 instead of 42
...
{    
    if (isfp16) {
        softargmax_kernel_half_depth<g_H_Hand, g_W_Hand><<<dim3(N, C, 1), dim3(W), 0, stream>>>(
            reinterpret_cast<const __half2 *>(inputs[0]), reinterpret_cast<__half *>(outputs[0]),
            reinterpret_cast<__half *>(outputs[0]) + C, reinterpret_cast<__half *>(outputs[0]) + 2 * C,
            reinterpret_cast<__half *>(outputs[1]), H * W, C * H * W, 
            __float2half(beta), __float2half(epsilon));
    } else {
        softargmax_kernel_float_depth<g_H_Hand, g_W_Hand><<<dim3(N, C, 1), dim3(W), 0, stream>>>(
            reinterpret_cast<const float *>(inputs[0]), reinterpret_cast<float *>(outputs[0]),
            reinterpret_cast<float *>(outputs[0]) + C, reinterpret_cast<float *>(outputs[0]) + 2 * C, 
            reinterpret_cast<float *>(outputs[1]), H * W, C * H * W, beta, epsilon);
    }
}
